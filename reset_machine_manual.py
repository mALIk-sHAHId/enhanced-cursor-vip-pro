...
def run(translator=None):
    """Run the machine ID reset process"""
    try:
        resetter = MachineIDResetter(translator)
        return resetter.reset_machine_ids()
    except Exception as e:
        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('reset.reset_failed', error=str(e)) if translator else f'Reset failed: {str(e)}'}{Style.RESET_ALL}")
        return False
